...
class option {
   public:
    ...
    //
    // Utilities to create the options
    //
    static option check(bool val, call_back_t on_change = nullptr);

    static option
        spin(int64_t val, spin_range r, call_back_t on_change = nullptr);

    static option combo(std::string                        default_value,
                        std::initializer_list<std::string> others,
                        call_back_t                        on_change = nullptr);

    static option button(call_back_t on_change);

    static option string(std::string val, call_back_t on_change = nullptr);
